...
void print_usage(char invalidFlag) {
	...
	dprintf(1, "-i <count> %22s wait <count> seconds between sending each packet\n", "");
...
void print_output_loop_error(t_pingData *data) {
	...
	dprintf(1, "%d bytes from %s : icmp_seq=%d %s", data->retPrintSize, reverseDNS, stats.pingNb, data->error);
	if (data->options & 1) {
		...
		dprintf(1, "%s\n", dstAddr);
		dprintf(1, "ICMP: type %hhd, code %hhu, size %lu, id 0x%hx, seq 0x%04hx\n", data->recievedBytesArray[28+20],
